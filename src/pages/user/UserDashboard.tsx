...
import { Calendar, ShoppingBag, UtensilsCrossed, ArrowRight, Loader2, CreditCard, CheckCircle, Clock, Users, MapPin, MessageSquare, Package } from "lucide-react";
          ...
          {reservations.length === 0 ? (
            ...
            <div className="space-y-4">
              {reservations.slice(0, 5).map((reservation) => (
                <div key={reservation.id} className="p-4 rounded-lg bg-secondary border border-border/50 hover:border-accent/50 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <p className="font-semibold text-foreground">Table {reservation.table_number}</p>
                          {reservation.capacity && (
                            <span className="text-xs text-muted-foreground">(Capacity: {reservation.capacity})</span>
                          )}
                        </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-muted-foreground">{formatDate(reservation.reservation_date)}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Clock className="w-4 h-4 text-muted-foreground" />
                      <span className="text-muted-foreground">{formatTime(reservation.reservation_time)}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm">
                      <Users className="w-4 h-4 text-muted-foreground" />
                      <span className="text-muted-foreground">{reservation.guests} guests</span>
                    </div>
                    {reservation.duration && (
                      <div className="flex items-center gap-2 text-sm">
                        <Clock className="w-4 h-4 text-muted-foreground" />
                        <span className="text-muted-foreground">{reservation.duration} min</span>
                      </div>
                    )}
                  </div>
          ...
          {orders.length === 0 ? (
            ...
            <div className="space-y-4">
              {orders.slice(0, 5).map((order) => (
                <div key={order.id} className="p-4 rounded-lg bg-secondary border border-border/50 hover:border-accent/50 transition-colors">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-lg bg-accent/20 flex items-center justify-center">
                        <ShoppingBag className="w-6 h-6 text-accent" />
                      ...
                      <div>
                        <p className="font-semibold text-foreground mb-1">Order #{order.id.slice(0, 8).toUpperCase()}</p>
                          ...
                          <div key={idx} className="flex items-center gap-3 text-sm">
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-xs text-muted-foreground">
                                {item.category} • Qty: {item.quantity} × ${parseFloat(item.price || 0).toFixed(2)}
                              </p>
                            </div>
